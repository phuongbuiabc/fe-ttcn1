...
export default function SuppliersPage() {
  ...
  useEffect(() => {
    const cached = supplierService.getCachedSuppliers();
    if (cached) {
      setSuppliers(cached);
      setLoading(false);
      fetchSuppliers(false); // Background update
    } else {
      fetchSuppliers(true);
    }
  }, []);
    ...
    <div className="space-y-6 pb-20 bg-[#fbfcfd] min-h-screen -m-6 p-6 ">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight font-headline uppercase">Nhà cung cấp
